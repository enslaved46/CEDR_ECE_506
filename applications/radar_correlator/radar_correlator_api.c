...
int main(void)
{
    ...
    // FFT 1 Input
    // getting input
    // samples from wave gen
    // getting real and img
    ...
    // FFT 2, input
    // like variable C
    // real and img part is getting place into FFT2 input
    ...
    //gsl_fft_wrapper(fft_inp, fft_out, len, true);
    // same input as FFT is being passed here , i.e sash_re_filt_type
    
    DASH_FFT_flt(fft_inp, fft_out2, len, true);

    // output of FFT is placed here, again, first placement X1
    /* for (size_t i = 0; i < len; i++) { */
    ...
    // output of FFT1 (input c) and FFT2(input D) is multiplied
    // only even , why ?
    /* for (i = 0; i < 2 * len; i += 2) { */
